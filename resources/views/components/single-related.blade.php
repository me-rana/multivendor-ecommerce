<div class="row" style="border-bottom: 1px solid gainsboro;padding-bottom: 10px;">
                        ...
                        <div class="col-md-8" style="padding:0">
                            <a  href="{{route('product.details', $product->slug)}}">
                                ...
                                <p style="color: #333;font-size: 14px;" class="product-title">
                                    {{$product->title}}
                                </p>
                            </a> 
                            <p style="color: #333;font-size: 14px;">
                                Price:@if($product->discount_price>0 || $product->price)
             <span><strong style="color: var(--optional_color)">{{ setting('CURRENCY_CODE_MIN') ?? 'TK' }}.{{$product->price ?? $product->discount_price}}</strong> <del>{{ setting('CURRENCY_CODE_MIN') ?? 'TK' }}.{{$product->regular_price}}</del></span>
            @else
               <span><strong style="color: var(--optional_color)">{{ setting('CURRENCY_CODE_MIN') ?? 'TK' }}.{{$product->regular_price}}</strong></span>
            @endif</p>
