...
@section('title', 'importExporter List')
...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1>importExporter List</h1>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="{{routeHelper('dashboard')}}">Home</a></li>
                    <li class="breadcrumb-item active">importExporter List</li>
                </ol>
            </div>
        </div>
    </div><!-- /.container-fluid -->
...
<section class="content">
    ...
    <div class="card">
        <div class="card-header">
            <div class="row">
                <div class="col-sm-6">
                    <h3 class="card-title">importExporter List</h3>
                </div>
                <div class="col-sm-6 text-right">
                    <a href="{{routeHelper('import-exporter/create')}}" class="btn btn-success">
        ...
        <div class="card-body">
            <table id="example1" class="table table-bordered table-striped">
                ...
                <tbody>
                    @foreach ($importExporters as $key => $data)
                        <tr>
                            <td>{{$key + 1}}</td>
                            <td>{{$data->name}}</td>
                            <td>{{$data->username}}</td>
                            ...
                            <td style="position:relative;">
                                ...
                                @endif
                                <a href="{{ route('admin.importExporter.ImportExportList',$data->id) }}" class="btn btn-primary btn-sm">
                                    <i class="fas fa-store"></i>
                                </a>
                                <a href="{{ routeHelper('import-exporter/'. $data->id) }}" class="btn btn-primary btn-sm">
                                    <i class="fas fa-eye"></i>
                                </a>
                                <a href="{{ routeHelper('import-exporter/'.$data->id.'/edit') }}" class="btn btn-info btn-sm">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="javascript:void(0)" data-id="{{$data->id}}" id="deleteData" class="btn btn-danger btn-sm"">
                                    <i class="nav-icon fas fa-trash-alt"></i>
                                </a>
                                <form id="delete-data-form-{{$data->id}}" action="{{ routeHelper('import-exporter/'. $data->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                </form>
                                ...
                                <div id="action_apply_vn_{{$data->id}}" style="display:none;background:var(--primary_color);padding:7px 10px;position:absolute;bottom:-30px;right:100%;width:90%;border-radius:4px;z-index:9999;">
                                    <a style="color:var(--secondary_color);" href="{{ route('admin.importExporter.change_pass_index', ['id'=>$data->id]) }}">Change Password</a>
                                </div>
                            </td>
                        </tr>
                    @endforeach
            ...
            {{ $importExporters->firstItem() }} - {{ $importExporters->lastItem() }} of {{ $importExporters->total() }} results
            <nav aria-label="Page navigation example">
                <ul class="pagination">
                    {{-- First Page Button --}}
                    <li class="page-item">
                        <a class="page-link" href="{{ $importExporters->url(1) }}">First</a>
                    </li>
            
                    {{-- Page Numbers --}}
                    @php
                        $totalPages = ceil($importExporters->total() / $importExporters->perPage());
                        $currentPage = $importExporters->currentPage();
                    ...
                    @if ($totalPages > 3)
                        ...
                        @for ($i = max($currentPage - 2, 2); $i < $currentPage; $i++)
                            <li class="page-item {{ $currentPage == $i ? 'active' : '' }}">
                                <a class="page-link" href="{{ $importExporters->url($i) }}">{{ $i }}</a>
                            </li>
                        @endfor
                        ...
                        {{-- Last Page Button --}}
                        <li class="page-item">
                            <a class="page-link" href="{{ $importExporters->url($totalPages) }}">Last</a>
                        </li>
                    @else
                        ...
                        @endfor
                    @endif
                    @if ($importExporters->nextPageUrl())
                            <li class="page-item">
                                <a class="page-link" href="{{ $importExporters->nextPageUrl() }}" aria-label="Next">
