...
<div class="container product-page" style="margin-top: 10px">
        ...
        <div class="products col-md-9">
            ...
            <div class="container">
                <div class="row" style="margin-bottom: 10px;">
                    @php 
                        $campaign_sliders = App\Models\CampaignImage::where('campaign_id',$campaign->id)->get();
                    @endphp
                    @push('css')
                    ...
                    @if($campaign_sliders->count() > 0)
                    <div class="swiper mySwiper">
                        <div class="swiper-wrapper">
                          <div class="swiper-slide">
                            <img src="{{ asset('uploads/campaign/'.$campaign->cover_photo) }}" />
                        </div>

                        @foreach ($campaign_sliders as $key => $slider)
                        <div class="swiper-slide">
                            <img src="{{asset('uploads/campaign/'.$slider->image)}}" />
                        </div>
                        @endforeach
                    ...
                    @else
                    <img src="{{ asset('uploads/campaign/'.$campaign->cover_photo) }}" class="cam_responsive_image" />
                    @endif
                ...
                <div class="row " id="grid-view">
                    @forelse ($products as $product)
                        ...
                        <div class="product px-3 col-lg-{{ 12/$product_setup->others_lg }} col-md-{{ 12/$product_setup->others_md }} col-sm-{{ 12/$product_setup->others_extra_sm }} col-{{ 12/$product_setup->others_sm }}">
    <?php 
       $typeid=$product->slug;
    ?>
    ...
    <div class="product-wrapper"   @if(setting('is_point')==1) style="height: 320px;" @endif>
        <div class="pin">
            ...
            <div class="container">
                <h6><strong style="color: var(--MAIN_MENU_ul_li_color) !important">{{ setting('CURRENCY_CODE_MIN') ?? 'TK' }}.{{$product->cprice ?? $product->discount_price}}</strong> <del>{{ setting('CURRENCY_CODE_MIN') ?? 'TK' }}.{{$product->regular_price}}</del></h6>
             <a href="{{route('product.details', $product->slug)}}">
                <h5 class="product-title" style="color: var(--optional_color); font-size: 18px !important;">{{implode(' ', array_slice(explode(' ', $product->title), 0, 10))}}...</h5>
            </a>
            </div>
        </div>
        <div class="home-add2">
           ...
           <div class="cbtn">
                ...
                @csrf
                    ...
                    <button style="margin-top: 5px; color: var(--MAIN_MENU_ul_li_color)" class="redirect" type="submit" title="Wishlist"><i class="fal fa-heart" aria-hidden="true"></i> </button>
                </form>
           </div>
        </div>
    </div>
...
<div class="bef-footer" style="border-radius: 5px;margin-bottom: 20px;padding: 10px; box-shadow: 0 1px 1px rgba(0, 0, 0, 0.1);">
